...
export default function History() {
  ...
  const handleDownloadDayPDF = (dateStr, dayRemarks) => {
    if (!dayRemarks || dayRemarks.length === 0) {
      toast.error('No remarks to export for this day.');
      return;
    }
    setIsGenerating(true);
    try {
      const columns = ['#', 'Student Name', 'Register No', 'Department', 'Year', 'Semester', 'Remark', 'Time'];
      const rows = dayRemarks.map((r, i) => [
        i+1, 
        r.name, 
        r.registerNumber||r.register_number, 
        r.department, 
      ...
      doc.text('MODERN INSTITUTE COLLEGE', doc.internal.pageSize.getWidth()/2, 15, { align: 'center' });
      doc.setFontSize(14);
      doc.text(`Daily Remarks Report`, doc.internal.pageSize.getWidth()/2, 25, { align: 'center' });
      
      doc.setFontSize(10);
      doc.text(`Date: ${formattedDate}`, 15, 35);
      doc.text(`Generated By: ${localStorage.getItem('userName') || 'Incharge'}`, 15, 42);
                ...
                <div className="overflow-y-auto flex-1">
                  {sortedDates.length > 0 ? sortedDates.map(dateStr => (
                    <div key={dateStr} className="mb-0">
                      <div className="px-6 py-2 bg-surface-container-low border-b border-outline-variant/20 font-label text-[12px] font-bold text-on-surface sticky top-0 z-10 shadow-sm flex items-center justify-between">
